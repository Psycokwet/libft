...
/*   Updated: 2021/08/27 11:53:43 by scarboni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_lstdbcpy_int(t_list_double *src, t_list_double **dst, void *(*copy_create_content)(void*))
{
	if (!src)
		return (SUCCESS);
...
static void	ft_lstdbcpy_int_for_prev(t_list_double *prev, t_list_double *current)
{
	if(!current)
		return ;
	current->prev = prev;
...
int	ft_lstdbcpy(t_list_double *head_src, t_list_double **head_dst, void *(*copy_create_content)(void*))
{
	if (!head_src)
		return (ERROR_WHILE_COPYING);
